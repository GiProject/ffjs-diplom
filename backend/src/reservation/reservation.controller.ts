...
export class ReservationController {
  ...
  @Roles('client')
  @UseGuards(RoleGuard)
  async add(@User() user: UserEntity, @Body() body: any) {
    const hotelRoom = await this.HotelRoomModel.findOne({
      _id: body.roomId,
    }).exec();
    ...
    return await this.reservationService.addReservation({
      ...
      dateEnd: new Date(
        formatInTimeZone(new Date(body.dateEnd), 'Europe/Moscow', 'yyyy-MM-dd'),
      ),
    });
  }
