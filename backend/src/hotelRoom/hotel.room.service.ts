...
export class HotelRoomService implements IHotelRoomService {
  ...
  public async findById(id: ID): Promise<HotelRoom> {
    return this.HotelRoomModel.findById(id).select(
  ...
  async search(params: SearchRoomsParams) {
    ...
    const count = await this.HotelRoomModel.find(params)
      .countDocuments()
      .exec();
  ...
  public async update(id: ID, data: UpdateHotelRoomParams): Promise<HotelRoom> {
    let resultImages;
